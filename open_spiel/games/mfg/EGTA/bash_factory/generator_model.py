...
BASE = 'python egta_example.py --game_name=mfg_crowd_modelling --meta_strategy_method=RD --oracle_type=BR --egta_iterations=25 --w_distance=0.015 --planning_iters=7 --fine_tune_iters=23'
ROOT = '--root_result_folder=baseline_mfg_crowd_modelling_RD'
BASE_2d = 'python egta_example.py --game_name=mfg_crowd_modelling_2d --meta_strategy_method=RD --oracle_type=BR --egta_iterations=25 --w_distance=0.015 --planning_iters=7 --fine_tune_iters=23'
ROOT_2d = '--root_result_folder=baseline_mfg_crowd_modelling_2d_RD'
SIZE = '--game_size='
...
def bash_factory_2d():
    ...
    for i, param in enumerate(params):
        ...
        with open(target, 'a') as file:
            commands = [BASE_2d, SIZE + str(10), HORIZON + str(t), IL_ + str(l),
                        ROOT_2d + str(l) + '_step' + str(t)]
            new_command = " ".join(commands)
            print(new_command)
            write_line(file, '\n')
            write_line(file, new_command)
