...
def train_model(model, train_loader, val_loader, epochs=100, lr=0.001, verbose=False):
    ...
    for epoch in range(epochs):
        ...
        if verbose and (epoch + 1) % 10 == 0:
            print(
                f'Epoch {epoch + 1}/{epochs}, Train Loss: {avg_train_loss:.6f}, Val Loss: {avg_val_loss:.6f}, Train R2: {train_r2:.6f}, Val R2: {val_r2:.6f}')
...
if __name__ == "__main__":
    ...
    batch_size = 32
    d_model = 128
    # 训练轮数由公式算出 Epochs = Training Steps *Batch Size /训练集数量(总样本数量-验证集数量)
    epochs = 304
    learning_rate = 0.001
    size = args.size
    step = args.step
    ...
    if args.data_dir:
        ...
        data_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "root_result",
                                "mfg_crowd_modelling_it_10_grid_True_sample_100_gendata_2026-04-06_20-29-38")
    ...
    train_strategies, val_strategies, train_mixtures, val_mixtures, train_utilities, val_utilities = train_test_split(
        strategies, mixtures, utilities, test_size=0.3, random_state=42
    )
